...
if [ -z "$INTERFACES" ]; then
   INTERFACES=`ip addr | grep $REDE -A0 | tail -n1 | awk '{print $2}' | cut -f1  -d'/'`
fi
...
[ -z "$LISTEN_TCP" ] || OPTIONS="$OPTIONS --listen-tcp=$INTERFACES:$LISTEN_TCP"
[ -z "$LISTEN_UDP" ] || OPTIONS="$OPTIONS --listen-udp=$INTERFACES:$LISTEN_UDP"
[ -z "$LISTEN_NG" ] || OPTIONS="$OPTIONS --listen-ng=$INTERFACES:$LISTEN_NG"
[ -z "$LISTEN_CLI" ] || OPTIONS="$OPTIONS --listen-cli=$INTERFACES:$LISTEN_CLI"
